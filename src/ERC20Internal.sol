...
abstract contract ERC20Internal is Context, IERC20Errors {
    ...
    // _name and _symbol are written during construction by PartyPoolExtraImpl1.init via
    // delegatecall through the shared PoolState storage layout (see PartyPoolStorage.sol).
    // Slither cannot follow the library/delegatecall handoff and so flags both detectors,
    // but the values are per-pool (set from constructor params) and therefore cannot be `constant`.
    ...
    function _spendAllowance(address owner, address spender, uint256 value) internal virtual {
        ...
        if (currentAllowance < type(uint256).max) {
            ...
            unchecked {
                // unchecked-safe: (1) subtraction guarded by the `currentAllowance < value`
                // revert immediately above, so currentAllowance >= value here.
