...
library LMSRStabilized {
    ...
    function swapAmountsForExactInput(
        ...
        // Compute r0 = exp((q_j - q_i) / b) = e_j / e_i directly using invB
        int128 r0 = _exp(qInternal[j].sub(qInternal[i]).mul(invB));
    ...
    function swapAmountsForPriceLimit(
        ...
        require(limitPrice > int128(0), "invalid limit");
    ...
    function swapAmountsForMint(
        ...
        // Precompute r0_j = exp((q_j - q_i) / b) = e_j / e_i for all j to avoid recomputing during search.
        int128[] memory r0 = new int128[](n);
        for (uint256 j = 0; j < n; ) {
            r0[j] = _exp(qInternal[j].sub(qInternal[i]).mul(invB));
            unchecked { j++; }
        }
        ...
        for (uint256 j = 0; j < n; ) {
            if (j != i) {
                ...
                if (aj > int128(0)) {
                    ...
                    // r0_j = exp((q_local[i] - q_local[j]) / b) = e_i / e_j
                    int128 r0_j = _exp(qLocal[i].sub(qLocal[j]).mul(invB));
    ...
    function price(int128 kappa, int128[] memory qInternal, uint256 baseTokenIndex, uint256 quoteTokenIndex) internal pure returns (int128) {
        ...
        int128 invB = ABDKMath64x64.div(ONE, b);
        // Marginal price quote / base = exp((q_quote - q_base) / b) = e_quote / e_base
