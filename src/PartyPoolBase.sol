...
// CHECKLIST: C.1 — single shared reentrancy lock (ReentrancyGuardTransient, EIP-1153) for every
// PartyPool mutating entry point. `nonReentrant` is applied on initialMint/mint/burn/swap/swapMint/
// burnSwap/collectProtocolFees in PartyPool.sol; this is the one lock that closes C.1 and C.5.
abstract contract PartyPoolBase is OwnableInternal, ERC20Internal, ReentrancyGuardTransient {
    ...
    // PartyPoolExtraImpl2 library (via the PoolState storage handle in
    ...
    // The write happens in PartyPoolExtraImpl1.init via delegatecall, which slither
    ...
    ///         Set in `PartyPool`'s constructor; read via `EXTCODECOPY` by the `_feeAt` /
    ///         `_basesArray` / `_feesArray` / `_bfPair` helpers below. Replaces the previous
    ...
    // delegatecall-init pattern: PartyPoolExtraImpl1 writes and reads `_initialized`
    ...
    // σ_swap defense state — see `doc/rate-limited-mints.md`.
    //
    // The mint deviation gate compares the current live σ_q (computed on demand
    // from `_lmsr.qInternal`) against `_sigmaSwap`, an EMA-anchored reference
    // updated at most once per block. The EMA target `_prevBlockEndSigmaQ` is
    // captured lazily at the entry of each new block's first state-changing
    // call: `_libComputeSizeMetric(qInternal)` is read from the pre-mutation
    // `qInternal` (which still reflects the previous block's end state at that
    // point) and stored, then `_sigmaSwap` steps one EMA shift toward it.
    //
    // The rate-limit accumulator `_gammaAccum` continuously decays via the same
    // EMA mechanism; `_gammaAccumLastBlock` carries the last decay timestamp.
    //
    // All five fields are written via `delegatecall` from PartyPoolExtraImpl1 /
    // PartyPoolExtraImpl1 through the PoolState mirror in PartyPoolStorage;
    // Slither cannot follow the indirection, hence the disables. The layout is
    // chosen so the (int128, uint64) pairs pack into single 32-byte slots.
    // slither-disable-next-line unused-state,constable-states
    int128 internal _sigmaSwap;            // Q64.64 — EMA-anchored gate reference (slot N, low 128 bits)
    // slither-disable-next-line unused-state,constable-states
    uint64  internal _lastUpdateBlock;     // block.number of last σ_swap step  (slot N, high 64 bits)
    // 64 bits of padding complete slot N.
    // slither-disable-next-line unused-state,constable-states
    int128 internal _prevBlockEndSigmaQ;   // Q64.64 — end-of-previous-block σ_q (slot N+1, low 128 bits)
    // slither-disable-next-line unused-state,constable-states
    uint64  internal _gammaAccumLastBlock; // block.number of last γ-accumulator decay (slot N+1, high 64 bits)
    // 64 bits of padding complete slot N+1.
    // slither-disable-next-line unused-state,constable-states
    int128 internal _gammaAccum;           // Q64.64 — continuously-decayed Σγ accumulator
    ...
    // Sum of all per-asset fees (PPM). Per-asset fees are capped < 10_000 by init,
    // and NUM_TOKENS is small, so the sum cannot overflow. Used by the swapMint /
    // burnSwap facades to derive the combined swap-leg fee (input + average other).
    function _sumFeesPpm() internal view returns (uint256 sum) {
        uint256 n = NUM_TOKENS;
        address store = IMMUTABLE_BFSTORE;
        uint256 feesBase = 1 + 32 * n;
        for (uint256 i = 0; i < n; ) {
            uint256 fee;
            assembly ("memory-safe") {
                extcodecopy(store, 0x00, add(feesBase, mul(i, 32)), 32)
            ...
            // unchecked-safe: (2)/(5) i++ bounded by n=NUM_TOKENS; per-token fee is a
            // ppm (< 1_000_000) so the running sum over a small basket cannot overflow.
            unchecked { sum += fee; i++; }
        }
    }
    ...
    // Read (baseI, baseJ, feeI, feeJ) from BFStore in a single assembly block.
    // Saves the function-call overhead of four separate _feeAt-style invocations
    // on the hot swap path. Single-element reads each clobber scratch (0x00..0x20).
    function _bfPair(uint256 i, uint256 j) internal view returns (
        uint256 baseI, uint256 baseJ, uint256 feeI, uint256 feeJ
    ) {
        address store = IMMUTABLE_BFSTORE;
        uint256 feesBase = 1 + 32 * NUM_TOKENS;
        assembly ("memory-safe") {
            ...
            baseI := mload(0x00)
            extcodecopy(store, 0x00, add(1, mul(j, 32)), 32)
            baseJ := mload(0x00)
            extcodecopy(store, 0x00, add(feesBase, mul(i, 32)), 32)
            feeI := mload(0x00)
            extcodecopy(store, 0x00, add(feesBase, mul(j, 32)), 32)
            feeJ := mload(0x00)
        }
    }
    ...
    // CHECKLIST: A.1, A.2, D.10 — the `msg.sender == payer` gates below are the authoritative
    //   defense against the free-`payer`/`from` allowance-drain (Uniswap-router) pattern. A different
    //   `receiver` (A.2) is harmless because this fires before any token/LP/accounting state moves.
    //   Reference callback-side gate for external integrators: src/PartyPoolCallbackVerifier.sol.
