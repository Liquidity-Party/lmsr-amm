...
library PartyPoolPermit2Witness {
    ...
    bytes32 internal constant SWAP_MINT_WITNESS_TYPEHASH = keccak256(
        "SwapMintWitness(address payer,address receiver,uint256 inputTokenIndex,uint256 lpAmountOut,uint256 maxAmountIn,uint256 deadline)"
    );

    string internal constant SWAP_MINT_WITNESS_TYPE_STRING =
        "SwapMintWitness witness)SwapMintWitness(address payer,address receiver,uint256 inputTokenIndex,uint256 lpAmountOut,uint256 maxAmountIn,uint256 deadline)TokenPermissions(address token,uint256 amount)";

    bytes32 internal constant MINT_WITNESS_TYPEHASH = keccak256(
        "MintWitness(address payer,address receiver,uint256 lpTokenAmount,uint256 deadline)"
    );

    string internal constant MINT_WITNESS_TYPE_STRING =
        "MintWitness witness)MintWitness(address payer,address receiver,uint256 lpTokenAmount,uint256 deadline)TokenPermissions(address token,uint256 amount)";
