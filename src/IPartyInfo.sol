...
interface IPartyInfo {
    ...
    ///         for token decimals.
    /// @dev Returns p_quote / p_base in Q128.128 format, scaled to external units by (denom_quote / denom_base).
    ...
    ///      because the initial deposit is handled by transferring tokens first, then calling mint().
