...
contract PartyPool is PartyPoolBase, OwnableExternal, ERC20External, IPartyPool {
    ...
    function balance(uint256 index) external view returns (uint256) { return _cachedUintBalances[index]; }
    ...
    constructor()
    {
        ...
        _tokens = p.tokens;
        KAPPA = p.kappa;
        require(p.fees.length == p.tokens.length, "fees length");
    ...
    function swap(
        ...
        // _receiveTokenFrom() will revert if the requested amount wasn't sent
