...
During market dislocation events, the first swap of any
asset in the pool reprices all the assets in the pool,
hardening them against further adverse selection.

Liquidity Party pools guarantee a bounded maximum loss to LP's of `κ*S*ln(N)` where `κ` is
...
1. Install the [Foundry](https://getfoundry.sh/) toolkit.
2. Update dependencies with `forge install`
3. Run `bin/mock` to launch a test environment running under `anvil` on `localhost:8545`. The mock environment will create several example pools along with mock ERC20 tokens that can be minted by anyone in any amount. See [DeployMock.sol](script/DeployMock.sol).

# Integration

Deployment addresses for each chain may be found in [liqp-deployments.json](deployment/liqp-deployments.json), and the `solc` output including ABI information is stored under `deployment/{chain_id}/v1/...`

The Liquidity Party pool factory is the [PartyPlanner](src/IPartyPlanner.sol) contract, which is a singleton per chain. The `PartyPlanner` contract not only deploys new pools but also indexes the pools and their tokens for easy metadata discovery. After a pool is created or discovered using the `PartyPlanner`, it can be used to perform swaps, minting, and other actions according to the [IPartyPool](src/IPartyPool.sol) interface. Due to contract size limitations, most view methods for prices and swaps are available from a separate singleton contract, [PartyInfo](src/IPartyInfo.sol).

User-facing swaps, mints, and burns should use the [PartyConcierge](src/PartyConcierge.sol), which has an ERC-7730 Clear Signing compatible interface at the cost of extra gas, about 10k for a typical swap. PartyConcierge also a singleton, so one approval covers all pools. Run [bin/gas-report](bin/gas-report) script for detailed gas costs.

# Implementation Notes

## Non-upgradable Proxy

Due to contract size constraints, the `PartyPool` contract uses `DELEGATECALL` to invoke implementations on the singleton [PartyPoolExtraImpl](src/PartyPoolExtraImpl.sol) and [PartyPoolMintImpl](src/PartyPoolMintImpl.sol) contracts. This proxy pattern is NOT upgradable and the implementation contract addresses used by the pool are immutable. Views implemented in `PartyInfo` have no delegation but simply accept the target pool as an argument, calculating prices etc. from public getters.
...
The fee for a swap is the input asset fee plus the output asset fee, but this total fee percentage is deducted from the gross output, after the LMSR calculation. The full input amount is priced by the kernel; the fee is then withheld from the gross output, so less of the output asset leaves the pool than the kernel priced. In this way, the LP holders accumulate fee value implicitly in their prorata share of the pool's total assets.

A swap-mint or burn-swap charges the full swap fee on its swap leg — the input asset fee plus the other assets' fees, exactly as the equivalent decomposed swap path would charge — while the proportional mint or burn leg is fee-free. Charging the full swap-leg fee closes a loophole in which an attacker could pair a compound operation with a fee-free proportional mint or burn and pay only one side's fee.
