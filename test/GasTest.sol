...
contract GasHarness is IPartySwapCallback {
    ...
    function swapApproval(
        ...
        (amountIn, amountOut, inFee) = pool.swap{value:msg.value}(address(this), fundingSelector, receiver, inputTokenIndex, outputTokenIndex, maxAmountIn, minAmountOut, deadline, unwrap, '');
    ...
    function swapPrefund(
        ...
        return pool.swap{value:msg.value}(address(this), fundingSelector, receiver, inputTokenIndex, outputTokenIndex, maxAmountIn, minAmountOut, deadline, unwrap, '');
    }
    ...
    function swapCallback(
        ...
        return pool.swap{value:msg.value}(address(this), this.liquidityPartySwapCallback.selector, receiver, inputTokenIndex, outputTokenIndex, maxAmountIn, minAmountOut, deadline, unwrap, '');
    }
...
contract GasTest is Test {
    ...
    function _doSwap(
        ...
        if (fundingSelector == Funding.APPROVAL)
            return harness.swapApproval{value:msg.value}(pool, pool.token(inputTokenIndex), payer, fundingSelector, receiver, inputTokenIndex, outputTokenIndex, maxAmountIn, minAmountOut, deadline, unwrap);
        if (fundingSelector == Funding.PREFUNDING) {
            pool.token(inputTokenIndex).transfer(address(harness), maxAmountIn);
            return harness.swapPrefund{value:msg.value}(pool, payer, fundingSelector, receiver, inputTokenIndex, outputTokenIndex, maxAmountIn, minAmountOut, deadline, unwrap);
        }
        else
            return harness.swapCallback{value:msg.value}(pool, payer, fundingSelector, receiver, inputTokenIndex, outputTokenIndex, maxAmountIn, minAmountOut, deadline, unwrap);
    }
    ...
    function _performSwapMintBurnSwapGasTest(IPartyPool testPool) internal {
        ...
        for (uint256 k = 0; k < iterations; k++) {
            ...
            (, uint256 minted, ) = testPool.swapMint(alice, Funding.APPROVAL, alice, 0, lpTarget, type(uint256).max, 0, bytes(""));
    ...
    function _performMintBurnGasTest(IPartyPool testPool) internal {
        ...
        for (uint256 k = 0; k < iterations; k++) {
            ...
            testPool.mint(alice, Funding.APPROVAL, alice, lpRequest, 0, bytes(""));
    ...
    function _doSwapPermit2(IPartyPool pool, uint256 inIdx, uint256 outIdx, uint256 maxAmountIn) internal {
        uint256 nonce = p2Nonce++;
        ...
        bytes32 tokenHash = keccak256(abi.encode(_TOKEN_PERMS_TH, address(pool.token(inIdx)), maxAmountIn));
        ...
        bytes memory cbData = abi.encode(nonce, deadline, abi.encodePacked(r, s, v));

        harness.swapPermit2(pool, aliceP2, aliceP2, inIdx, outIdx, maxAmountIn, deadline, cbData);
    }

    function _performSwapGasTestPermit2(IPartyPool testPool) internal {
        uint256 maxIn = 10_000;
        for (uint256 i = 0; i < 20; i++) {
            if (i % 2 == 0) _doSwapPermit2(testPool, 0, 1, maxIn);
            else _doSwapPermit2(testPool, 1, 0, maxIn);
            maxIn *= 787;
            maxIn /= 1000;
        }
    }
    ...
    function testSwapGasPermit210() public {
        _performSwapGasTestPermit2(pool10P2);
    }

    function testSwapGasPermit220() public {
        _performSwapGasTestPermit2(pool20P2);
    }
