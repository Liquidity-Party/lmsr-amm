...
contract MidpointBSweep is Test {
    ...
    /// Inlined Hanson must match the library output for the same state and input.
    /// (Library kernel is frozen-b Hanson; midpoint-b is no longer the production formula.)
    function test_inlined_matches_library() public view {
        int128 a = q10[0] >> 7;
        int128 inlinedY = _swapHanson(kappa20pct, sumQ10, q10[0], q10[1], a);
        ...
        assertLt(
            uint256(int256(diff)),
            4,
            "inlined Hanson must match library"
        );
    }
