...
contract Permit2Test is Test {
    ...
    function testSwapMintPermit2HappyPath() public {
        ...
        PartyPoolPermit2Witness.SwapMintWitness memory w = PartyPoolPermit2Witness.SwapMintWitness({
            ...
            minLpOut: 0, partialFillAllowed: false,
            deadline: txDeadline
        });
        bytes memory cbData = abi.encode(nonce, sigDeadline, _sign(_singleDigest(
            address(pool), address(token0), maxAmountIn, nonce, sigDeadline,
        ...
        (uint256 amountIn, uint256 lpMinted, , ) = pool.swapMint(
            alice, Funding.PERMIT2, alice,
            0, lpAmountOut, maxAmountIn, 0, false, txDeadline, cbData
        );
    ...
    ///         Permit2 caps are the signed `maxAmountsIn` (the witness commits to them via
    ///         `maxAmountsInHash`); the actual proportional `depositAmounts` floats with state
    ///         and is pulled within those caps.
    function testMintPermit2HappyPath() public {
        ...
        // Use real per-token caps (slightly above the expected proportional deposit) so the
        // signature survives small state drift between signing and execution and remains
        // compatible with partial fills. The Permit2 batch is signed over the *caps*, not
        // over `deposits`.
        uint256[] memory maxAmountsIn = new uint256[](3);
        for (uint256 i = 0; i < 3; i++) {
            maxAmountsIn[i] = deposits[i] + deposits[i] / 100 + 1; // +1% headroom
        }
        ...
        bytes memory cbData = abi.encode(nonce, sigDeadline, _sign(_batchDigest(
            address(pool), tokAddrs, maxAmountsIn, nonce, sigDeadline,
        ...
        vm.prank(bob);
        (uint256 actualLp, ) = pool.mint(alice, Funding.PERMIT2, alice, lpToMint, maxAmountsIn, 0, false, txDeadline, cbData);

        assertTrue(actualLp > 0, "received LP tokens");
        assertEq(pool.balanceOf(alice), aliceLP0 + actualLp, "alice received LP");
        assertEq(token0.balanceOf(alice), aliceBefore0 - deposits[0], "alice paid exactly deposits[0]");
        assertEq(token1.balanceOf(alice), aliceBefore1 - deposits[1], "alice paid exactly deposits[1]");
        assertEq(token2.balanceOf(alice), aliceBefore2 - deposits[2], "alice paid exactly deposits[2]");
    }

    /// @notice Regression for the cap/actual mismatch fix: a third-party state change between
    ///         signing and execution must not brick a Permit2 mint, because the user signs caps
    ///         and the pool requests only the (smaller) proportional deposit.
    function testMintPermit2SurvivesFrontrunSwap() public {
        uint256 lpToMint = pool.totalSupply() / 100;
        uint256 nonce = 20;
        uint256 sigDeadline = block.timestamp + 1 hours;
        uint256 txDeadline  = block.timestamp + 1 hours;

        uint256[] memory depositsBefore = info.mintAmounts(pool, lpToMint);
        IERC20[]  memory allToks  = pool.allTokens();
        address[] memory tokAddrs = new address[](allToks.length);
        for (uint256 i = 0; i < allToks.length; i++) tokAddrs[i] = address(allToks[i]);
        ...
        uint256[] memory maxAmountsIn = new uint256[](3);
        for (uint256 i = 0; i < 3; i++) {
            maxAmountsIn[i] = depositsBefore[i] + depositsBefore[i] / 50 + 1; // +2% headroom
        }
        ...
        // Attacker performs a tiny swap that changes _cachedUintBalances. Pre-fix this would
        // have changed the recomputed `depositAmounts` and produced an InvalidSigner revert.
        address attacker = makeAddr("attacker");
        token0.mint(attacker, 1e6);
        vm.startPrank(attacker);
        token0.approve(address(pool), type(uint256).max);
        pool.swap(attacker, Funding.APPROVAL, attacker, 0, 1, 1e6, 0, txDeadline, false, "");
        vm.stopPrank();

        vm.prank(bob);
        (uint256 actualLp, ) = pool.mint(alice, Funding.PERMIT2, alice, lpToMint, maxAmountsIn, 0, false, txDeadline, cbData);
        assertTrue(actualLp > 0, "mint survives front-run state change");
    }
    ...
    function testMintPermit2WitnessBindsLpAmount() public {
        ...
        PartyPoolPermit2Witness.MintWitness memory w = PartyPoolPermit2Witness.MintWitness({
            payer: alice, receiver: alice,
            lpTokenAmount: lpToMint,
            maxAmountsInHash: keccak256(abi.encodePacked(new uint256[](3))),
            minLpOut: 0, partialFillAllowed: false,
            deadline: txDeadline
        });
        bytes memory cbData = abi.encode(nonce, sigDeadline, _sign(_batchDigest(
        ...
        vm.prank(bob);
        vm.expectRevert(); // real Permit2 reverts with InvalidSigner
        pool.mint(alice, Funding.PERMIT2, alice, lpToMint * 2, new uint256[](3), 0, false, txDeadline, cbData);
    }
