...
contract PartyPoolPriceTest is PartyPoolBase {
    ...
    function testInitialMintCustomLpTokensDoesNotAffectSwaps() public {
        ...
        (IPartyPool poolDefault, uint256 lpDefault) = Deploy.newPartyPool2("LP_DEFAULT", "LP_DEFAULT", tokens, kappaDefault, feePpm, false, INIT_BAL, 0);
        ...
        (IPartyPool poolCustom, uint256 lpCustom) = Deploy.newPartyPool2("LP_CUSTOM", "LP_CUSTOM", tokens, kappaCustom, feePpm, false, INIT_BAL, customLpAmount);
    ...
    function testProportionalMintingScaledByInitialAmount() public {
        ...
        (IPartyPool poolDefault, uint256 lpDefault) = Deploy.newPartyPool2("LP_DEFAULT", "LP_DEFAULT", tokens, kappaDefault2, feePpm, false, INIT_BAL, 0);
        ...
        (IPartyPool poolCustom,) = Deploy.newPartyPool2("LP_CUSTOM", "LP_CUSTOM", tokens, kappaCustom2, feePpm, false, INIT_BAL, customLpAmount);
        ...
        (uint256 mintedDefault, ) = poolDefault.mint(alice, Funding.APPROVAL, alice, lpRequestDefault, new uint256[](3), 0, false, 0, bytes(""));
        (uint256 mintedCustom, ) = poolCustom.mint(alice, Funding.APPROVAL, alice, lpRequestCustom, new uint256[](3), 0, false, 0, bytes(""));

        uint256 expectedRatio = (mintedCustom * 1000) / mintedDefault;
        uint256 actualRatio = (scaleFactor * 1000);

        // Deposits for the same mint % are identical across the two pools (asserted
        // above), and the custom pool's supply is exactly scaleFactor× the default's,
        // so its proportional mint yields exactly scaleFactor× the LP. Pure integer
        // accounting — no rounding. Measured: mintedCustom/mintedDefault = 3.000 exact.
        assertEq(expectedRatio, actualRatio, "Minted LP ratio must exactly match the scale factor");
    ...
    function testInitialPoolPriceIsOne() public {
        ...
        // poolPrice of a freshly-initialized balanced pool is exactly the decimals
        // scale factor; the only rounding is the single Q64.64 div above. Measured
        // diff: 0. Bound at 2 ULP of Q64.64 (the div's floor), not the prior 1e-9
        // (≈1.8e10 ULP).
        int128 tol = int128(2);
        int128 diff = ratio.sub(expectedRatio).abs();
        assertLe(diff, tol, "poolPrice(token0) should be 1.0 to within 2 ULP");
        ...
        if (!allZero) {
            pool.mint(alice, Funding.APPROVAL, alice, lpRequest, new uint256[](3), 0, false, 0, bytes(""));
        }
        vm.stopPrank();
        ...
        // A proportional mint preserves the pool price exactly, so price/priceAfter
        // is 1.0 up to the single Q64.64 div's floor. Measured diff: 0. Bound at
        // 2 ULP, not the prior 1e-6 (≈1.8e13 ULP).
        tol = int128(2);
        diff = ratio.sub(expectedRatio).abs();
        assertLe(diff, tol, "Pool price must remain 1.0 after a proportional mint (within 2 ULP)");
    }
    ...
    function testPoolPriceWhenToken0HasThreeTimesToken1() public {
        ...
        (IPartyPool poolCustom, ) = Deploy.newPartyPoolWithDeposits(
            "LP3X_POOLPRICE", "LP3X_POOLPRICE", tokens, kappa, feePpm, false, deposits, INIT_BAL * 6 * 10**18
        );
        ...
        // poolPrice is proportional to reserves, so for deposits [3·INIT, INIT, 2·INIT]
        // the price ratio p0/p1 is exactly 3 up to the single Q64.64 div's floor.
        // Measured diff: 0. Bound at 2 ULP, not the prior 1e-6 (≈1.8e13 ULP).
        int128 tol = int128(2);
        int128 diff = ratio.sub(expectedRatio).abs();
        assertLe(diff, tol, "poolPrice(token0)/poolPrice(token1) must be 3.0 to within 2 ULP");
    }

    /// @notice Create a 3-token pool where token0 has 3x the balance of token1 and verify
    /// that the buy price of token1 in token0 equals 3.
    function testPriceWhenToken0HasThreeTimesToken1() public {
        ...
        (IPartyPool poolCustom, ) = Deploy.newPartyPoolWithDeposits("LP3X", "LP3X", tokens, kappa, feePpm, false, deposits, 0);
        ...
        uint256 price = info.price(poolCustom, 0, 1);
        uint256 expected = 3 << 128;
        assertEq(uint256(uint128(price)), uint256(uint128(expected)), "Buy price of token1 in token0 should be 3.0000000");
    }
    ...
    function testPriceMatchesSwapForSmallInput() public {
        ...
        (IPartyPool poolCustom, ) = Deploy.newPartyPoolWithDeposits("LP3X_SWAP", "LP3X_SWAP", tokens, kappa, feePpm, false, deposits, 0);

        uint256 infoPrice = info.price(poolCustom, 1, 0);
        uint256 inputAmount = INIT_BAL/100;

        // Fee-on-output: swapAmounts returns (amountIn=grossIn, netOut, outFee).
        // Average kernel price = grossIn / (netOut + outFee) = grossIn / grossOut.
        (uint256 grossIn, uint256 amountOut, uint256 outFee) = info.swapAmounts(poolCustom, 1, 0, inputAmount);

        uint256 grossOut = amountOut + outFee;
        require(grossIn > 0 && grossOut > 0, "inputs must be positive for price comparison");

        int128 swapPrice = ABDKMath64x64.divu(grossIn, grossOut);

        uint256 slippage = (uint256(int256(swapPrice)) << 64) - infoPrice;
        // The average price of a 1%-of-pool swap (kappa=100, near-flat curve)
        // exceeds the marginal info.price() only by finite-trade price impact.
        // Measured: 33 ppm of price. Bound at 100 ppm (infoPrice/10_000, ≈3×
        // headroom). The prior 4<<(128-5) = 2^125 was 37.5% of price — ~10⁴× loose.
        uint256 tol = infoPrice / 10_000; // 100 ppm of price
        assertTrue(slippage <= tol, "avg swap price diverged from marginal info.price by >100 ppm");
    }

    /// @notice Ensure average buy prices are monotonically non-decreasing with larger inputs.
    function testPricesMonotoneIncreasingWithLargerInputs() public {
        IERC20[] memory tokens = new IERC20[](3);
        tokens[0] = IERC20(address(token0));
        tokens[1] = IERC20(address(token1));
        tokens[2] = IERC20(address(token2));
        ...
        (IPartyPool poolCustom, ) = Deploy.newPartyPoolWithDeposits(
            "LP_MONO", "LP_MONO", tokens, kappa, feePpm, false, deposits, 0
        );
        ...
        // Fee-on-output: average kernel price = grossIn / (amountOut + outFee).
        // feePpm=0 here so outFee=0 and grossOut == amountOut; formula reduces to grossIn/amountOut.
        int128 p_eps;
        {
            (uint256 grossIn, uint256 amountOut, uint256 outFee) = info.swapAmounts(poolCustom, base, quote, eps);
            uint256 grossOut = amountOut + outFee;
            require(grossIn > 0 && grossOut > 0, "nonzero quote");
            p_eps = ABDKMath64x64.divu(grossIn, grossOut);
        }

        int128 p_a1;
        {
            (uint256 grossIn, uint256 amountOut, uint256 outFee) = info.swapAmounts(poolCustom, base, quote, a1);
            uint256 grossOut = amountOut + outFee;
            require(grossIn > 0 && grossOut > 0, "nonzero quote");
            p_a1 = ABDKMath64x64.divu(grossIn, grossOut);
        }

        int128 p_a2;
        {
            (uint256 grossIn, uint256 amountOut, uint256 outFee) = info.swapAmounts(poolCustom, base, quote, a2);
            uint256 grossOut = amountOut + outFee;
            require(grossIn > 0 && grossOut > 0, "nonzero quote");
            p_a2 = ABDKMath64x64.divu(grossIn, grossOut);
        }

        int128 p_a3;
        {
            (uint256 grossIn, uint256 amountOut, uint256 outFee) = info.swapAmounts(poolCustom, base, quote, a3);
            uint256 grossOut = amountOut + outFee;
            require(grossIn > 0 && grossOut > 0, "nonzero quote");
            p_a3 = ABDKMath64x64.divu(grossIn, grossOut);
        }

        // No downward slack: the average buy price is strictly increasing in trade
        // size, so each step is a clean >= with margin. Measured step gaps (Q64.64):
        // p_eps-marginal +2.05e16, a1-eps +8.2e16, a2-a1 +1.02e17, a3-a2 +8.2e17 —
        // all far above any rounding. The prior 4e-5 slack (≈7.4e14) was never used.
        assertGe(uint256(int256(p_eps)) << 64, p0, "p(eps) must be >= marginal price");
        assertGe(p_a1, p_eps, "p(a1) must be >= p(eps)");
        assertGe(p_a2, p_a1,  "p(a2) must be >= p(a1)");
        assertGe(p_a3, p_a2,  "p(a3) must be >= p(a2)");
    }
}
/* solhint-enable */
