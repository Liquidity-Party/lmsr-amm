...
contract ChecklistSectionK is Test {
    ...
    /// CHECKLIST: K.3 — planner-side rejection now happens per-call inside `newPool`
    ///                  (no chain-wide protocolFeeAddress setter remains). The
    ///                  `Planner: zero fee address` revert is covered by
    ///                  `test/PartyPlanner.t.sol::test_newPool_immutables_validation`.
    ...
    function testChecklist_K5_killLeavesBurnsWorking() public {
        ...
        (IPartyPool fresh,) = Deploy.newPartyPoolWithDeposits(
            "K5", "K5", tokens, kappa, 1000, false, deposits, 1e18
        );
        ...
        vm.expectRevert(bytes("killed"));
        fresh.swapMint(alice, Funding.APPROVAL, alice, 0, 1, type(uint256).max, 0, false, 0, "");
        vm.stopPrank();

        // burn() must still work after kill (no trapped-funds risk).
        uint256 totalLp = fresh.totalSupply();
        uint256[] memory withdrawn = fresh.burn(address(this), bob, totalLp, new uint256[](2), 0, false);
