...
contract PartyConciergeTest is Test {
    ...
    function setUp() public {
        ...
        concierge = new PartyConcierge(planner, new PartyInfo(), IPermit2(MOCK_PERMIT2), 500, 0, 300);
    ...
    function testMintViaConcierge() public {
        ...
        (uint256 minted, ) = concierge.mint(pool, alice, lpRequest, new uint256[](3), 0, false, 0, false);
        vm.stopPrank();
    ...
    function testMintPullsFromCaller() public {
        ...
        concierge.mint(pool, alice, lpRequest, new uint256[](3), 0, false, 0, false);
        vm.stopPrank();

        assertLt(token0.balanceOf(alice), t0Before, "alice token0 not spent");
    }

    /// @notice mint maxAmountsIn cap is enforced (MEV/slippage protection).
    function testMintMaxAmountsInEnforced() public {
        uint256 lpRequest = pool.totalSupply() / 100;
        ...
        // Set a cap below the actual required deposit on token0.
        uint256[] memory maxIn = new uint256[](3);
        maxIn[0] = 1;
        vm.expectRevert(bytes("slippage control"));
        concierge.mint(pool, alice, lpRequest, maxIn, 0, false, 0, false);
        vm.stopPrank();
    }

    /// @notice mint minLpOut floor is enforced.
    function testMintMinLpOutEnforced() public {
        uint256 lpRequest = pool.totalSupply() / 100;
        ...
        vm.expectRevert(bytes("slippage control"));
        concierge.mint(pool, alice, lpRequest, new uint256[](3), type(uint256).max, false, 0, false);
        vm.stopPrank();
    }
    ...
    function testBurnViaConcierge() public {
        ...
        uint256[] memory withdrawn = concierge.burn(pool, address(this), lpAmount, new uint256[](3), 0, false);
    ...
    /// @notice burn minAmountsOut floor is enforced.
    function testBurnMinAmountsOutEnforced() public {
        uint256 lpAmount = pool.totalSupply() / 10;
        pool.approve(address(concierge), lpAmount);

        uint256[] memory minOut = new uint256[](3);
        minOut[0] = type(uint256).max;
        vm.expectRevert(bytes("slippage control"));
        concierge.burn(pool, address(this), lpAmount, minOut, 0, false);
    }
    ...
    function testBurnRevertsForUnsupportedPool() public {
        ...
        concierge.burn(IPartyPool(address(fakeToken)), address(this), 1, new uint256[](3), 0, false);
    }
    ...
    function testSwapMintViaConcierge() public {
        ...
        (uint256 amountIn, uint256 lpMinted,,) = concierge.swapMint(
            pool, IERC20(address(token0)), alice, lpTarget, SWAP_AMT, 0, false, 0, false
        );
        vm.stopPrank();
    ...
    function testSwapMintMaxAmountInEnforced() public {
        // maxAmountIn too low → slippage control.
        uint256 lpTarget = pool.totalSupply() / 100;
        vm.startPrank(alice);
        token0.approve(address(concierge), type(uint256).max);
        vm.expectRevert(bytes("slippage control"));
        concierge.swapMint(pool, IERC20(address(token0)), alice, lpTarget, 1, 0, false, 0, false);
        vm.stopPrank();
    }

    function testSwapMintMinLpOutEnforced() public {
        // minLpOut floor too high → slippage control on the LP-out side.
        uint256 lpTarget = pool.totalSupply() / 10_000;
        vm.startPrank(alice);
        token0.approve(address(concierge), type(uint256).max);
        vm.expectRevert(bytes("slippage control"));
        concierge.swapMint(pool, IERC20(address(token0)), alice, lpTarget, SWAP_AMT, type(uint256).max, false, 0, false);
        vm.stopPrank();
    }
    ...
    function testMintRecipientReceivesLp() public {
        ...
        (uint256 minted, ) = concierge.mint(pool, bob, lpRequest, new uint256[](3), 0, false, 0, false);
        vm.stopPrank();
    ...
    function testBurnRecipientReceivesBasket() public {
        ...
        uint256[] memory withdrawn = concierge.burn(pool, bob, lpAmount, new uint256[](3), 0, false);
    ...
    function testConciergeReentrantCallReverts() public {
        ...
        (IPartyPool malPool,) = Deploy.newPool(
            planner,
            "MalPool", "MP",
            toks, kappa, uint256(1000),
            address(this), address(this),
            deposits, INIT_BAL * 3, 0
        );
    ...
    function testSwapMintRecipientReceivesLp() public {
        ...
        (, uint256 minted,,) = concierge.swapMint(
            pool, IERC20(address(token0)), bob, lpTarget, SWAP_AMT, 0, false, 0, false
        );
        vm.stopPrank();
