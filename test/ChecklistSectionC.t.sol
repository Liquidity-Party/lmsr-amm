...
///                                       re-entry into mint / burn from inside swap's
///                                       callback all revert.
...
contract ReentrancyProbe {
    ...
    function fundingCallback(bytes32, /*nonce*/ IERC20 token, uint256 amount, bytes calldata /*cbData*/) external {
        ...
        if (action == Action.REENTER_SWAP) {
            ...
            POOL.mint(address(this), Funding.APPROVAL, address(this), 1, new uint256[](POOL.allTokens().length), 0, false, 0, "");
        } else if (action == Action.REENTER_BURN) {
            POOL.burn(address(this), address(this), 1, new uint256[](POOL.allTokens().length), 0, false);
...
contract ChecklistSectionC is Test {
    ...
    function setUp() public {
        ...
        (pool,) = Deploy.newPartyPoolWithDeposits(
            "C", "C", tokens, kappa, 1000, false, deposits, INIT_BAL * 3
        );
    ...
    function testChecklist_C3_erc777PoolDeploysAndReentryBlocked() public {
        ...
        (IPartyPool hookPool,) = Deploy.newPool(
            planner2,
            "HOOK-LP", "HKLP",
