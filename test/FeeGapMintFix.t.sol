...
contract FeeGapMintFixTest is Test {
    ...
    function _mintApproval(address who, uint256 lpRequest) internal returns (uint256) {
        ...
        (uint256 minted, ) = pool.mint(who, Funding.APPROVAL, who, lpRequest, new uint256[](3), 0, false, 0, "");
        vm.stopPrank();
        return minted;
    }
    ...
    function testIncumbentPerLpSizeDoesNotDecrease() public {
        ...
        // A proportional mint may lose at most a few ULP of Q64.64 to ceil-rounding
        // of the per-asset deposits; the fee-gap bug lost far more (visibly >0.1%).
        // Measured here: per-LP size actually *rose* by ~6.5e6 ULP (no loss). Bound
        // the downward slack at 8 ULP — amply covers the few-ULP worst case while
        // staying ~1e8× tighter than the prior 0.01% (≈1.8e9 ULP) slack.
        int128 tolerance = int128(8);
        assertGe(perLpAfterMint, perLpAfterSwaps.sub(tolerance),
            "mint must not dilute incumbent per-LP size beyond a few ULP");
    }
