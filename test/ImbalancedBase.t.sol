...
///
/// The model: an attacker who can mint arbitrary amounts of any TestERC20
/// to themselves on demand (the TestERC20 contract has unlimited `mint`,
/// giving the attacker effectively unbounded inventory for ALL tokens).
///
...
abstract contract ImbalancedBase is Test {
        ...
        internal returns (DeployedPool memory dp)
    {
        ...
        (dp.pool, ) = Deploy.newPartyPoolWithDeposits(
            "ImbLP", "ImbLP", ierc20s, kappa, swapFeePpm, false, deposits, 0
        );
        ...
        internal view returns (Snapshot memory s)
    {
        ...
        for (uint256 i = 0; i < dp.n; i++) {
            ...
            try dp.info.price(dp.pool, numeraireIdx, i) returns (uint256 p) {
                s.pricesQ128[i] = p;
            } catch {
                s.pricesValid = false;
            }
    ...
    /// @notice I-1: pool.allTokens()[i].balanceOf(pool) == cached[i] + protocolOwed[i].
    function _assertI1(DeployedPool memory dp, string memory tag) internal view {
        ...
        for (uint256 i = 0; i < dp.n; i++) {
            uint256 actual   = dp.pool.allTokens()[i].balanceOf(address(dp.pool));
    ...
    function _assertNoExtraction(
        ...
        // (B) Attacker value at pre-cycle prices.
        // Tolerance: ABS_EXTRACTION_TOL_WEI absorbs cumulative Q64.64 rounding from
        // multi-leg cycles (swapMint / burnSwap / swap each do ABDK mul/div which
        // can produce sub-wei rounding drift in either direction). The constant is
        // intentionally tiny — anything beyond it is a real extraction signal.
        ...
        uint256 ABS_EXTRACTION_TOL_WEI = 2;
        assertLe(
            vAfter,
            vBefore + ABS_EXTRACTION_TOL_WEI,
