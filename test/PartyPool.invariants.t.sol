...
contract PartyPoolInvariantHandler is CommonBase, StdCheats, StdUtils, StdAssertions {
    ...
    function swap(
        ...
        (uint256 amountIn, uint256 amountOut, uint256 inFee) = pool.swap(
    ...
    function mint(uint256 actorSeed, uint256 lpAmount) external {
        ...
        _snapshotProtocolFees();
        vm.prank(actor);
        pool.mint(actor, Funding.APPROVAL, actor, lpAmount, 0, "");
    ...
    function burn(uint256 actorSeed, uint256 fracSeed) external {
        ...
        _snapshotProtocolFees();
        vm.prank(actor);
        pool.burn(actor, actor, lpAmount, 0, false);
    ...
    function swapMint(uint256 actorSeed, uint256 inputIdx, uint256 lpFracSeed) external {
        ...
        try pool.swapMint(actor, Funding.APPROVAL, actor, inputIdx, lpAmountOut, type(uint256).max, 0, "")
            returns (uint256, uint256 l, uint256 f)
        { lpMinted = l; inFee = f; }
    ...
    /// CHECKLIST: H.6 — closes the "PREFUNDING race" pattern: the auth gate fires
    /// before any balance check, so an attacker cannot front-run a prefunded swap
    /// and consume the deposit by spoofing `payer`.
    ...
    ///         PartyPoolMintImpl.sol:75 (`require(msg.sender == payer)`).
    function mintAllowanceTheftAttempt(
        ...
        vm.prank(attacker);
        try pool.mint(victim, Funding.APPROVAL, attacker, lpAmount, 0, "") {
    ...
    ///         Must revert at PartyPoolMintImpl.sol:75.
    function swapMintAllowanceTheftAttempt(
        ...
        vm.prank(attacker);
        try pool.swapMint(victim, Funding.APPROVAL, attacker, inputIdx, lpTarget, maxIn, 0, "") {
    ...
    ///         with msg.sender != payer. Must revert at PartyPoolMintImpl.sol:78.
    ...
    function mintPrefundTheftAttempt(
        ...
        vm.prank(attacker);
        if (useSwapMint) {
            try pool.swapMint(victim, Funding.PREFUNDING, attacker, inputIdx, swapMintLp, type(uint256).max, 0, "") {
                assertTrue(false, "I-15 VIOLATION: swapMint prefund theft succeeded");
            } catch { }
        } else {
            try pool.mint(victim, Funding.PREFUNDING, attacker, lpAmount, 0, "") {
    ...
    ///         Must revert at PartyPoolMintImpl.sol:323-324 (allowance underflow on
    ///         `allowed - lpAmount`).
    function burnLpTheftAttempt(
        ...
        vm.prank(attacker);
        try pool.burn(victim, attacker, lpAmount, 0, false) {
    ...
    ///         Must revert at PartyPoolMintImpl.sol:622-623 (allowance underflow).
    ...
    function quoteExecuteParity(
        ...
        assertEq(aFee,       qFee,       "I-6: inFee differs between quote and execute");
...
contract PartyPoolInvariantsTest is StdInvariant, Test {
    ...
    /// CHECKLIST: A.1, A.2, H.6 — fuzz-invariant proof that swap(PREFUNDING) rejects every
    /// attacker permutation of `payer`/`receiver` (gate at PartyPoolBase.sol:216).
    /// Closure for H.6 (PREFUNDING race / front-run consumption of prefunded swap):
    /// the same `msg.sender == payer` gate that rejects allowance theft also rejects
    /// any third party from claiming a prefunded deposit, since the funding-mode
    /// dispatch in `_receiveTokenFrom` shares the gate across PREFUNDING/APPROVAL.
    ...
    /// @notice burn(full LP balance) for any actor must return at least
    ///         floor(lpBalance * cached[i] / totalSupply) − tolerance[i] of each asset.
    ///
    /// tolerance[i] = cached[i] / 2^64 + 1  (two sequential ABDK Q64.64 floor ops).
    function invariant_I4_lpSolvency() public view {
        ...
        for (uint256 a = 0; a < actors.length; a++) {
            ...
            // Pre-compute floor proportions; if all are zero the invariant
            // trivially holds and we skip burnAmounts() to avoid "too small" reverts
            // that the pool emits when proportional withdrawals round to zero.
            bool hasNonZeroFloor = false;
            uint256[] memory floors = new uint256[](n);
            for (uint256 i = 0; i < n; i++) {
                floors[i] = (lpBal * cached[i]) / totalSupply;
                if (floors[i] > 0) hasNonZeroFloor = true;
            }
            if (!hasNonZeroFloor) continue;

            uint256[] memory withdrawAmounts = info.burnAmounts(pool, lpBal);

            for (uint256 i = 0; i < n; i++) {
                uint256 tolerance   = cached[i] / (2 ** 64) + 1;
                uint256 minExpected = floors[i] > tolerance ? floors[i] - tolerance : 0;
                assertGe(withdrawAmounts[i], minExpected,
                    "I-4: burn returns less than floor proportion minus Q64.64 tolerance");
            }
        }
    }
    ...
    /// PartyPoolMintImpl.sol:75; `receiver=attacker` is harmless because the gate
    ...
    /// CHECKLIST: A.1, A.2 — same gate (PartyPoolMintImpl.sol:75) for swapMint.
    ...
    /// CHECKLIST: A.1, A.2 — same gate (PartyPoolMintImpl.sol:78) for PREFUNDING
    /// on mint and swapMint, blocking front-run-and-claim of victim's prefund.
    ...
    /// allowance (PartyPoolMintImpl.sol:323-324); without it the call reverts on
    ...
    /// CHECKLIST: A.1, A.2 — same LP-allowance gate (PartyPoolMintImpl.sol:622-623)
