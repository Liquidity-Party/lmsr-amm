...
//        uint256 swapFeePpm = 1000; // 0.1%
    ...
    function testSwapWBTCtoSHIB() public {
        ...
        // Get price before swap: How much WBTC you need to buy 1 SHIB (buy semantics)
        // price(input, output) returns input/output, so price(0, 1) = WBTC per SHIB
        ...
        // Get price after swap: How much WBTC you need to buy 1 SHIB (buy semantics)
        ...
        assertTrue(fee <= amountInUsed, "Fee should not exceed input (fee is on output)");
        ...
        // Verify price increased after swap: WBTC cost per SHIB rises after buying SHIB
        assertTrue(priceAfter > priceBefore, "WBTC cost per SHIB should increase after buying SHIB");

        // Calculate the average buy price of the swap (input per output) in Q128.128 format
        // Average buy price = amountInUsed / amountOut, scaled by 2^128 to match price() format
        uint256 averagePrice = (amountInUsed << 128) / amountOut; // WBTC per SHIB in Q128.128

        // Verify the average buy price is greater than the initial marginal price
        assertTrue(averagePrice > priceBefore, "Average buy price should be greater than marginal price before");
        ...
        // For large trades in imbalanced pools, the realized average can legitimately deviate from
    ...
    function testPriceRatio() public view {
        // Buy price: cost in SHIB to buy 1 WBTC (SHIB per WBTC, buy semantics)
        uint256 shibPerWbtc = info.price(pool, 1, 0);

        // Buy price: cost in WBTC to buy 1 SHIB (WBTC per SHIB, buy semantics)
    ...
    function testSwapAmountsQuery() public view {
        ...
        (uint256 grossIn, uint256 amountOut, uint256 fee) = info.swapAmounts(
            pool,
            0,           // tokenInIndex (WBTC)
            1,           // tokenOutIndex (SHIB)
            inputAmount  // maxAmountIn
    ...
    function testSwapSHIBtoWBTC() public {
        ...
        // Get price before swap: How much SHIB you need to buy 1 WBTC (buy semantics)
        // price(input, output) returns input/output, so price(1, 0) = SHIB per WBTC
        ...
        // Get price after swap: How much SHIB you need to buy 1 WBTC (buy semantics)
        ...
        // Verify price increased after swap: SHIB cost per WBTC rises after buying WBTC
        assertTrue(priceAfter > priceBefore, "SHIB cost per WBTC should increase after buying WBTC");

        // Calculate the average buy price of the swap (input per output) in Q128.128 format
        // Average buy price = amountInUsed / amountOut, scaled by 2^128 to match price() format
        uint256 averagePrice = (amountInUsed << 128) / amountOut; // SHIB per WBTC in Q128.128
    ...
    function testPriceMarginalAccuracy() public view {
        // Test WBTC -> SHIB price (buy semantics: WBTC per SHIB)
        {
            uint256 tinyAmount = 1; // 1 satoshi of WBTC
            uint256 marginalPrice = info.price(pool, 0, 1); // WBTC per SHIB in Q128.128 (buy semantics)

            // Get swap amounts for tiny input
            (uint256 amountIn, uint256 amountOut, ) = info.swapAmounts(
                pool,
                0,           // tokenInIndex (WBTC)
                1,           // tokenOutIndex (SHIB)
            ...
            // Calculate average buy price from tiny swap in Q128.128 format
            uint256 swapPrice = (amountIn << 128) / amountOut;

            // The average price of a tiny swap differs from the instantaneous
            // marginal price by finite-trade-size price impact plus integer
            // quantization. With only a 1-satoshi WBTC input this is dominated
            // by quantization and is the most sensitive case here. Measured:
            // 9 ppm. Bound at 50 ppm (≈5× headroom for quantization swings),
            // not the prior 1% (10_000 ppm, ~1000× loose).
            uint256 tolerance = marginalPrice / 20_000; // 50 ppm
            uint256 diff = swapPrice > marginalPrice ? swapPrice - marginalPrice : marginalPrice - swapPrice;
            assertTrue(diff <= tolerance, "WBTC->SHIB: swap price diverged from marginal by >50 ppm");
        }

        // Test WETH -> SHIB price (buy semantics: WETH per SHIB)
        {
            uint256 tinyAmount = 1e15; // 0.001 WETH (18 decimals)
            uint256 marginalPrice = info.price(pool, 2, 1); // WETH per SHIB in Q128.128 (buy semantics)

            // Get swap amounts for tiny input
            (, uint256 amountOut, ) = info.swapAmounts(
                pool,
                2,           // tokenInIndex (WETH)
                1,           // tokenOutIndex (SHIB)
                tinyAmount   // maxAmountIn
            ...
            // Calculate average buy price from tiny swap in Q128.128 format
            uint256 swapPrice = (tinyAmount << 128) / amountOut;

            // 0.001 WETH input — large enough that quantization is negligible,
            // so the marginal-vs-average gap is just finite-trade price impact.
            // Measured: 1 ppm. Bound at 10 ppm (10× headroom).
            uint256 tolerance = marginalPrice / 100_000; // 10 ppm
            uint256 diff = swapPrice > marginalPrice ? swapPrice - marginalPrice : marginalPrice - swapPrice;
            assertTrue(diff <= tolerance, "WETH->SHIB: swap price diverged from marginal by >10 ppm");
        }
    }
    ...
    function testPrefundingThreeSwapsInRow() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            // Get price before swap: How much WETH you need to buy 1 SHIB (buy semantics)
            // price(input, output) returns input/output, so price(2, 1) = WETH per SHIB
            ...
            (uint256 amountInUsed, uint256 amountOut, uint256 fee) = pool.swap(
                alice,              // payer (must equal msg.sender; tokens already pre-transferred to pool)
            ...
            // Get price after swap: How much WETH you need to buy 1 SHIB (buy semantics)
            ...
            // Verify price increased after swap: WETH cost per SHIB rises after buying SHIB
            assertGt(priceAfter, priceBefore, "WETH cost per SHIB should increase after buying SHIB");

            // Calculate the average buy price of the swap (input per output) in Q128.128 format
            // Average buy price = amountInUsed / amountOut, scaled by 2^128 to match price() format
            uint256 averagePrice = (amountInUsed << 128) / amountOut; // WETH per SHIB in Q128.128

            // Verify the average buy price is greater than the initial marginal price
            assertGt(averagePrice, priceBefore, "Average buy price should be greater than marginal price before");
            ...
            // We verify the fundamental property that price moved in the expected direction.
            assertGt(priceAfter, priceBefore, "Price should have moved (already verified above)");
