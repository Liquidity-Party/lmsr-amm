...
contract PartyPoolTest is PartyPoolBase {
    ...
    function testProportionalMintZeroLpReverts() public {
        ...
        pool.mint(alice, Funding.APPROVAL, alice, 0, new uint256[](3), 0, false, 0, bytes(""));
        vm.stopPrank();
    }
    ...
    function testProportionalMintOneWeiSucceedsAndProtectsPool() public {
        ...
        pool.mint(alice, Funding.APPROVAL, alice, lpAmount, new uint256[](3), 0, false, 0, bytes(""));
    ...
    function testBurnFullRedemption() public {
        ...
        pool.burn(address(this), bob, totalLp, new uint256[](3), 0, false);
    ...
    function testMintDepositAmountsMatchesMint_3TokenPool() public {
        ...
        for (uint k = 0; k < requests.length; k++) {
            ...
            if (!allZero) {
                ...
                pool.mint(alice, Funding.APPROVAL, alice, req, new uint256[](3), 0, false, 0, bytes(""));
    ...
    function testMintDepositAmountsMatchesMint_10TokenPool() public {
        ...
        for (uint k = 0; k < requests.length; k++) {
            ...
            if (!allZero) {
                pool10.mint(alice, Funding.APPROVAL, alice, req, new uint256[](10), 0, false, 0, bytes(""));
    ...
    function testBurnReceiveAmountsMatchesBurn_3TokenPool() public {
        ...
        for (uint k = 0; k < burns.length; k++) {
            ...
            if (myLp < req) {
                ...
                token1.approve(address(pool), type(uint256).max);
                token2.approve(address(pool), type(uint256).max);
                pool.mint(alice, Funding.APPROVAL, address(this), topUp, new uint256[](3), 0, false, 0, bytes(""));
                vm.stopPrank();
            }
            ...
            pool.burn(address(this), bob, req, new uint256[](3), 0, false);

            // Burn value clamp can consume a small number of wei per token due to
            // Q64.64 mul/div rounding in the proportional accounting path. The
            // tolerance is scaled by amount magnitude (roughly 1 ULP at Q64.64 per
            // step, accumulated across burnAmounts vs the on-chain burn's clamp).
            uint256 tol0 = expected[0] / 1_000_000 + 16;
            uint256 tol1 = expected[1] / 1_000_000 + 16;
            uint256 tol2 = expected[2] / 1_000_000 + 16;
            assertApproxEqAbs(token0.balanceOf(bob) - b0Before, expected[0], tol0, "token0 withdraw mismatch");
            assertApproxEqAbs(token1.balanceOf(bob) - b1Before, expected[1], tol1, "token1 withdraw mismatch");
            assertApproxEqAbs(token2.balanceOf(bob) - b2Before, expected[2], tol2, "token2 withdraw mismatch");
    ...
    function testBurnReceiveAmountsMatchesBurn_10TokenPool() public {
        ...
        for (uint k = 0; k < burns.length; k++) {
            ...
            if (myLp < req) {
                ...
                token8.approve(address(pool10), type(uint256).max);
                token9.approve(address(pool10), type(uint256).max);
                pool10.mint(alice, Funding.APPROVAL, address(this), topUp, new uint256[](10), 0, false, 0, bytes(""));
                vm.stopPrank();
            }
            ...
            pool10.burn(address(this), bob, req, new uint256[](10), 0, false);

            // Per-token tolerance scaled by amount magnitude — burn value clamp
            // can shave 1 ULP per Q64.64 op even when σ_swap = σ_live in theory.
            for (uint256 i = 0; i < 10; i++) {
                uint256 actual_i;
                if (i == 0) actual_i = token0.balanceOf(bob) - beforeBal[0];
                else if (i == 1) actual_i = token1.balanceOf(bob) - beforeBal[1];
                else if (i == 2) actual_i = token2.balanceOf(bob) - beforeBal[2];
                else if (i == 3) actual_i = token3.balanceOf(bob) - beforeBal[3];
                else if (i == 4) actual_i = token4.balanceOf(bob) - beforeBal[4];
                else if (i == 5) actual_i = token5.balanceOf(bob) - beforeBal[5];
                else if (i == 6) actual_i = token6.balanceOf(bob) - beforeBal[6];
                else if (i == 7) actual_i = token7.balanceOf(bob) - beforeBal[7];
                else if (i == 8) actual_i = token8.balanceOf(bob) - beforeBal[8];
                else actual_i = token9.balanceOf(bob) - beforeBal[9];
                uint256 tol = expected[i] / 1_000_000 + 16;
                assertApproxEqAbs(actual_i, expected[i], tol, "withdraw mismatch");
            }
