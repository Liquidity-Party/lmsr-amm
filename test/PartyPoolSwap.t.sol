...
contract PartyPoolSwapTest is PartyPoolBase {
    ...
    ///         P_fwd close to the requested floor when executed via swap().
    function testSwapAmountsForExactPrice() public {
        uint256 target = info.price(pool, 0, 1) * 999 / 1000; // 0.1% below current
        ...
        assertTrue(fee <= amountIn, "fee <= amountIn");
        ...
        uint256 tol = target / 1000; // 0.1% tolerance for integer rounding
        assertTrue(priceAfter <= target + tol, "price must not exceed target after fill");
        assertTrue(priceAfter >= target - tol, "price must reach close to target");
    }
    ...
    function testSwapMintBasic() public {
        ...
        (, uint256 minted, ) = pool.swapMint(alice, Funding.APPROVAL, alice, 0, lpTarget, budget, 0, bytes(""));
    ...
    function testSwapMintLargeInputPartial() public {
        ...
        (uint256 lpTarget,,) = info.maxLpForBudget(pool, 0, budget);
        assertTrue(lpTarget > 0, "precondition: large budget can mint some LP");
    ...
    function testSwapMintMinLpOutEnforced() public {
        ...
        (, uint256 minted,) = pool.swapMint(alice, Funding.APPROVAL, alice, 0, lp1, 10_000, 0, bytes(""));
        ...
        token0.approve(address(pool), type(uint256).max);
        (, uint256 minted2,) = pool.swapMint(alice, Funding.APPROVAL, alice, 0, lp2, 10_000, 0, bytes(""));
    ...
    function testSwapMintMinLpOutQuoteExact() public {
        ...
        (uint256 amountIn, uint256 minted,) = pool.swapMint(alice, Funding.APPROVAL, alice, 0, lpTarget, qIn, 0, bytes(""));
        vm.stopPrank();
    ...
    function testSwapMintMinLpOutReverts() public {
        ...
        pool.swapMint(alice, Funding.APPROVAL, alice, 0, lpTarget, 1, 0, bytes(""));
    ...
    /// @notice Buying token1 with token0 must decrease price(0,1); buying token0 with token1 must increase it.
    function testSwapMovesMarginalPriceInExpectedDirection() public {
        ...
        uint256 priceAfter = info.price(pool, 0, 1);
        assertTrue(priceAfter < priceBefore, "price(0,1) must decrease after buying token1 with token0");
        ...
        assertTrue(priceAfterReverse > priceBeforeReverse, "price(0,1) must increase after buying token0 with token1");
    }
}
/* solhint-enable */
