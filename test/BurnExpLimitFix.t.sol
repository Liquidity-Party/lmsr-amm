...
contract BurnExpLimitFix is Test {
    ...
    function _expectedExpLimitPayout(
        ...
        // b is anchored to the pre-burn Σq (effectiveSigmaQ) on every leg — the
        // closed-cycle-arbitrage fix freezes b for all intra-block reads so that
        // swap, swapMint, and burnSwap price against the same surface.
        int128 sizeMetric = q[i].add(q[j]);        // 2-token: Σq = q[0] + q[1]
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //  Test 3a: High-α regime where LP-favor ROUNDING pushes inner ≥ ONE
    // ═══════════════════════════════════════════════════════════════════════

    /// @notice Regression for the audit finding "burnSwap EXP_LIMIT inner ≥ ONE
    ///         skips a real payout leg". The prior implementation aborted the
    ///         sub-swap leg whenever `innerSat >= ONE`, on the (incorrect)
    ///         premise that the only way to land there was via a degenerate
    ///         no-op. In fact, when α is large enough that aj/b approaches
    ///         qDiff/b in the saturated regime, the +1 ulp ceil bump on
    ///         expNegA (LP-favor rounding) and the floored expNegQDiff sum
    ///         to slightly more than ONE — and the correct payout is
    ///             y = qDiff + b·ln(inner)   ≈ qDiff  (because ln(1+ε) ≈ ε,
    ///                                                  and ε is sub-ulp · b)
    ///         which is a SIGNIFICANT positive contribution (qDiff is on the
    ///         order of qLocal[i]/2 here, not a rounding crumb). Skipping it
    ///         silently underpays the burner by a large fraction (≈25–30% in
    ///         the auditor's E2E PoC). This test exercises the same regime
    ///         at the kernel level.
    function testUnit_BurnExpLimit_HighAlpha_InnerGteOne_PaysLeg() public {
        // Pool: q[0] = 100, q[1] = 42 (ratio 0.42 > 1−α = 0.4, the
        // threshold for the +1-ulp ceil to push inner above ONE).
        int128 kappa = ABDKMath64x64.divu(1, 200); // 0.005
        int128[] memory q = new int128[](2);
        q[0] = ABDKMath64x64.fromUInt(100);
        q[1] = ABDKMath64x64.fromUInt(42);

        int128 alpha = ABDKMath64x64.divu(3, 5); // 0.6 — large LP burn

        _initSkewed(q, kappa);

        // Pin preconditions: this case MUST exercise the EXP_LIMIT branch
        // AND the rounded `innerSat ≥ ONE` corner. If a future ABDK or
        // kernel change moves the regime, fail loud rather than silently
        // re-testing the normal path.
        int128 oMA = ONE.sub(alpha);
        int128 qiLocal = q[0].mul(oMA);
        int128 qjLocal = q[1].mul(oMA);
        int128 sizeMetric = q[0].add(q[1]); // effectiveSigmaQ for kernel-level call
        int128 b = kappa.mul(sizeMetric);
        int128 invB = ABDKMath64x64.div(ONE, b);
        int128 qDiff = qiLocal.sub(qjLocal);
        int128 qDiffOverB = qDiff.mul(invB);
        assertGt(qDiffOverB, EXP_LIMIT, "precondition: EXP_LIMIT branch must fire");

        // Reconstruct innerSat with the same primitive sequence as the
        // production code (including the +1 ulp ceil on expNegA), so the
        // precondition `innerSat ≥ ONE` is checked against the actual
        // implementation surface rather than an idealized model.
        int128 aj = alpha.mul(q[1]);
        int128 expArg = aj.mul(invB);
        int128 expNegA = ABDKMath64x64.exp(expArg.neg());
        if (expNegA < int128(type(int128).max)) {
            expNegA = expNegA + 1; // LP-favor ceil; matches kernel L838
        }
        int128 oneMinusExpNegA = ONE.sub(expNegA);
        int128 expNegQDiff = ABDKMath64x64.exp(qDiffOverB.neg());
        int128 innerSat = oneMinusExpNegA.add(expNegQDiff);
        assertGt(innerSat, ONE, "precondition: rounded innerSat must be > ONE");
        ...
        uint256 q0Wei = ABDKMath64x64.mulu(q[0], 1e18);
        uint256 directShareWei = ABDKMath64x64.mulu(alpha.mul(q[0]), 1e18);
        // The skipped leg is approximately qDiff in token units. With
        // q[0]=100, q[1]=42, α=0.6: qDiff = 0.4·(100-42) = 23.2. The
        // direct share α·q[0] = 60. Fair payout ≈ 60 + 23.2 = 83.2.
        // The previous bug returned exactly 60.
        uint256 expectedLegWei = ABDKMath64x64.mulu(qDiff, 1e18);

        // (a) Fix must not drain the reserve.
        assertLt(payoutWei, q0Wei, "fix regression: payout >= reserve");

        // (b) Fix must NOT silently zero the leg. The leg contribution is
        //     ≈ qDiff (≈ 23.2 ETH-scaled); allowing for sub-wei ABDK noise
        //     on b·ln(inner) we assert at least 99% of qDiff.
        uint256 minLeg = (expectedLegWei * 99) / 100;
        assertGt(payoutWei, directShareWei + minLeg, "BUG: inner>=ONE branch skipped real payout leg");

        // (c) The total must round LP-favor: payout ≤ direct + qDiff.
        //     `y = qDiff + b·ln(inner)` with inner only slightly > ONE
        //     gives a tiny positive correction; ABDK ln+mul floor that
        //     correction toward 0, so payout ≤ direct + qDiff to within
        //     ABDK rounding (a handful of ulp; 1e6 wei is generous).
        assertLe(payoutWei, directShareWei + expectedLegWei + 1e6, "fix overshoots qDiff: rounding broken");
    }
