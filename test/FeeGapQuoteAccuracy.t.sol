...
///
///         These tests assert that quoted amounts match executed amounts to the
///         wei (forward quoters are wei-exact) when fees have accumulated.
contract FeeGapQuoteAccuracyTest is Test {
    ...
    function testSwapMintQuoteMatchesExecutionAfterFees() public {
        ...
        (uint256 actualIn, uint256 lpMinted,,) = pool.swapMint(
            actor, Funding.APPROVAL, actor, 0, lpOut, quotedIn * 2, 0, false, 0, "");
        vm.stopPrank();
        ...
        // Wei-exact: swapMintAmounts is a forward quoter that calls the identical
        // kernel with the same fee-inclusive qInternal / effectiveSigmaQ that
        // swapMint executes against, with the same rounding direction, in the
        // same block (no intervening leg retains fees). Any divergence is the
        // stale-state bug this regression guards. Measured: 5054 == 5054.
        assertEq(quotedIn, actualIn,
            "swapMintAmounts quote must equal execution to the wei after fee accrual");
    }
    ...
    function testBurnSwapQuoteMatchesExecutionAfterFees() public {
        ...
        // Wei-exact: burnSwapAmounts forward-quotes through the same kernel with
        // the same fee-inclusive state and rounding as burnSwap, in the same
        // block. The minAmountOut convention requires the quote be a safe (<=)
        // lower bound on delivered output, so the invariant is actualOut >=
        // quotedOut; measured here it is exactly equal (9982 == 9982). assertEq
        // is the strictest form that still encodes "exec never under-delivers"
        // and catches the stale-state divergence this regression guards.
        assertEq(actualOut, quotedOut,
            "burnSwapAmounts quote must equal execution to the wei after fee accrual");
    }
    ...
    function testMaxLpForBudgetFitsAfterFees() public {
        ...
        (uint256 actualIn, uint256 lpMinted,,) = pool.swapMint(
            actor, Funding.APPROVAL, actor, 0, lpOut, budget, 0, false, 0, "");
        vm.stopPrank();
