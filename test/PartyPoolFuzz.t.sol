...
contract PartyPoolFuzzTest is PartyPoolBase {
    ...
    function testFuzz_swap(uint256 inputIdx, uint256 outputIdx, uint256 maxIn) public {
        uint256 n_ = pool.immutables().numTokens;
        ...
        assertLe(outFee,    amountIn, "swap: outFee > amountIn");
    ...
    function testFuzz_swap_quoteParity(uint256 inputIdx, uint256 outputIdx, uint256 maxIn) public {
            ...
            returns (uint256 aIn, uint256 aOut, uint256 aFee)
        {
            ...
            assertEq(aFee, qFee, "I-6 swap: outFee mismatch between quote and execute");
        } catch {
    ...
    function testFuzz_swap_lpFeeAccrual(uint256 inputIdx, uint256 outputIdx, uint256 maxIn) public {
        ...
        if (amountOut > 0) {
            assertGt(outFee, 0, "I-12: swap outFee must be > 0 when feePpm > 0 and amountOut > 0");
            uint256 protoShare = (outFee * Deploy.PROTOCOL_FEE_PPM) / 1_000_000;
            assertGt(outFee - protoShare, 0, "I-12: LP portion of swap outFee must be > 0");
        }
    }
    ...
    function testFuzz_mint(uint256 lpAmount) public {
        ...
        vm.prank(alice);
        (uint256 lpMinted, ) = pool.mint(alice, Funding.APPROVAL, alice, lpAmount, new uint256[](3), 0, false, 0, "");
    ...
    function testFuzz_mintBurnRoundTrip(uint256 lpAmount) public {
        ...
        vm.prank(alice);
        try pool.burn(alice, alice, lpMinted, new uint256[](3), 0, false) returns (uint256[] memory withdrawAmounts) {
            for (uint256 i = 0; i < 3; i++) {
    ...
    function testFuzz_burn(uint256 fracSeed) public {
        ...
        uint256[] memory withdrawAmounts = pool.burn(address(this), alice, lpBurn, new uint256[](3), 0, false);
    ...
    function testFuzz_swapMint(uint256 inputIdx, uint256 lpAmountOut) public {
        inputIdx = bound(inputIdx, 0, pool.immutables().numTokens - 1);
        ...
        try pool.swapMint(alice, Funding.APPROVAL, alice, inputIdx, lpAmountOut, type(uint256).max, 0, false, 0, "")
            returns (uint256 amountInUsed, uint256 lpMinted, uint256 inFee, uint256)
        {
    ...
    function testFuzz_swapMint_minLpOutReverts(uint256 inputIdx, uint256 lpAmountOut) public {
        inputIdx = bound(inputIdx, 0, pool.immutables().numTokens - 1);
        // Bound lpAmountOut to ≤ 1% of supply so a single-token mint of this size
        // doesn't trip the σ_swap deviation gate before the slippage check fires.
        // The slippage-guard property is what this fuzz is asserting.
        uint256 cap = pool.totalSupply() / 100;
        if (cap == 0) cap = 1;
        lpAmountOut = bound(lpAmountOut, 1, cap);
        ...
        pool.swapMint(alice, Funding.APPROVAL, alice, inputIdx, lpAmountOut, qIn - 1, 0, false, 0, "");
        vm.stopPrank();
    }
    ...
    function testFuzz_swapMint_quoteParity(uint256 inputIdx, uint256 lpAmountOut) public {
        ...
        try pool.swapMint(alice, Funding.APPROVAL, alice, inputIdx, lpAmountOut, qUsed, 0, false, 0, "")
            returns (uint256 aUsed, uint256 aLp, uint256 aFee, uint256)
        {
    ...
    function testFuzz_swapMintBurnSwapRoundTrip(uint256 inputIdx, uint256 lpAmountOut) public {
        ...
        inTok.approve(address(pool), type(uint256).max);
        try pool.swapMint(alice, Funding.APPROVAL, alice, inputIdx, lpAmountOut, type(uint256).max, 0, false, 0, "")
            returns (uint256 u, uint256 l, uint256, uint256)
        {
    ...
    function testFuzz_swap_10tokens(uint256 inputIdx, uint256 outputIdx, uint256 maxIn) public {
        uint256 n_ = pool10.immutables().numTokens;
        inputIdx  = bound(inputIdx,  0, n_ - 1);
        outputIdx = bound(outputIdx, 0, n_ - 1);
        if (inputIdx == outputIdx) outputIdx = (outputIdx + 1) % n_;
        maxIn = bound(maxIn, AMOUNT_MIN, INIT_BAL / 10);

        TestERC20 inTok  = TestERC20(address(pool10.allTokens()[inputIdx]));
        TestERC20 outTok = TestERC20(address(pool10.allTokens()[outputIdx]));
        inTok.mint(alice, maxIn);
        ...
        assertLe(outFee,    amountIn, "pool10 swap: outFee > amountIn");
    ...
    function testFuzz_swapMint_10tokens(uint256 inputIdx, uint256 lpAmountOut) public {
        ...
        try pool10.swapMint(alice, Funding.APPROVAL, alice, inputIdx, lpAmountOut, type(uint256).max, 0, false, 0, "")
            returns (uint256 amountInUsed, uint256 lpMinted, uint256 inFee, uint256)
        {
    ...
    function testFuzz_burn_10tokens(uint256 fracSeed) public {
        ...
        uint256[] memory withdrawAmounts = pool10.burn(address(this), alice, lpBurn, new uint256[](n_), 0, false);
