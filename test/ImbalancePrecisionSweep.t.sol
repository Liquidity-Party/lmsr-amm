...
/// @notice Sweep test: frozen-b Hanson cost-preservation as q values drift.
///
...
/// q values WILL drift from 1.0; this sweep verifies cost-preservation holds
...
///   1. Run a 1% swap with the production (frozen-b Hanson) kernel
///   2. Evaluate b_pre = κ·Σq at the pre-swap state
///   3. Measure |C(q_after) - C(q_before)| / |C| at b_pre
///
/// Frozen-b Hanson is exactly cost-preserving at b_pre by construction; the
/// residual is purely fixed-point rounding noise across all imbalance ratios.
contract ImbalancePrecisionSweepTest is Test {
    ...
    /// Fixed assertion: residual < 1e-6 (1 ppm) at R ≤ 100. Frozen-b Hanson is
    /// exactly cost-preserving at b_pre; residuals are fixed-point noise (~1e-12).
