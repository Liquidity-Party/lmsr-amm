...
The property is verified by exhaustive parametric grid testing in
\texttt{test/MidpointBSweep.sol} (tests
\texttt{test\_midpoint\_\allowbreak NeverOvershoots\_LSLMSR} and
\texttt{test\_midpoint\_\allowbreak NeverOvershoots\_\allowbreak Asymmetric\allowbreak Pool}).
The grid is intentionally pushed well past any production-realistic
envelope:
\begin{equation*}
  N \in \{2, 5, 10, 50, 100\}, \quad
  \kappa \in [10^{-4},\, 2.0], \quad
  a/q \in [10^{-4},\, 10^{-1}],
\end{equation*}
covering $\kappa$ over more than four orders of magnitude. The grid is
54 symmetric points plus a 3-point asymmetric grid that exercises the
load-bearing far-from-balanced regime where the Taylor approximation
variants could in principle leak. Tolerance: $10^{-10}$ absolute, well
below 1~wei in Q64.64. \textbf{No grid point overshoots.} The bound is
therefore established by a combination of the structural argument above
and exhaustive testing across a range substantially wider than any
deployment-recommended envelope, rather than by a closed-form proof over
the unbounded $(a, \mathbf{q}, \kappa, n)$ space.
...
(\texttt{LMSRKernel.\allowbreak swapAmountsForExactInput}) is a two-pass
midpoint approximation of the path-exact LS-LMSR mapping
(\S\ref{sec:lp-safety}).
...
empirically verified on a 54-point symmetric grid plus a 3-point
asymmetric grid, with tolerance $10^{-10}$ absolute --- substantially
wider than any production-realistic operating envelope. The symmetric
grid spans
\[
  N \in \{2, 5, 10, 50, 100\}, \quad
  \kappa \in [10^{-4},\, 2.0], \quad
...
calls \texttt{kill()} (disabling new deposits, swaps, flash loans, and
single-asset withdrawals) and rotates \texttt{protocolFeeAddress} so any
...
operation. \texttt{swap} and \texttt{swapMint} are \textbf{fee-on-input}:
the fee is deducted from the user-submitted input \emph{before} the
fee-free LMSR kernel runs, so the kernel prices the net (post-fee) input.
\texttt{burnSwap} is \textbf{fee-on-output}: the fee-free kernel computes
the gross payout first, then the fee is deducted from that payout before
the user receives it. The collected fee is retained in the pool, in the
token on the fee'd side, and split between the protocol share
(\S\ref{sec:protocol-fees}) and the LP share (kept in reserves, raising
$\Sm$ and the LP-share-denominated value).

\subsection{Per-Asset Swap Fees}

Each asset $k$ has an immutable fee rate $f_k$ set at deployment,
expressed in parts-per-million. The deployer enforces $f_k < 10{,}000$~ppm
($< 1\%$). \texttt{swap} is \textbf{fee-on-input}: for an asset-to-asset
swap $i \to j$, the effective pair fee is the additive composition
\begin{equation}
...
\]
The kernel prices $a_{\mathrm{eff}}$ and returns the corresponding output
$y$; the user receives exactly $y$ in token $j$, and the pool retains
$a_{\mathrm{user}} - a_{\mathrm{eff}}$ in token $i$ as the collected fee
(then split into protocol/LP shares).
...
that token's fee applies and there is no pair-fee additive composition.
The fee \emph{side} (input vs output) differs between the two operations
and matches the side on which the user moves tokens:

\begin{itemize}
  \item \texttt{swapMint} is \textbf{fee-on-input}, in the deposited
        token. The pool computes
        $a_{\mathrm{eff}} = a_{\mathrm{user}}\,(1 - f_i)$ and passes
        $a_{\mathrm{eff}}$ to the fee-free kernel; the kernel mints exactly
        $\Delta L$ LP shares for the user (exact-LP-out). The fee
        $a_{\mathrm{user}} - a_{\mathrm{eff}}$ is collected in the
        deposited token; the protocol share is split off and the
        remainder stays in pool reserves.
  \item \texttt{burnSwap} is \textbf{fee-on-output}, in the withdrawn
        token. The fee-free kernel computes the gross payout
        $y_{\mathrm{gross}}$ for the burned LP fraction; the user
        receives
        $y_{\mathrm{net}} = y_{\mathrm{gross}}\,(1 - f_j)$. The fee
        $y_{\mathrm{gross}} - y_{\mathrm{net}}$ is retained in the
        withdrawn token; the protocol share is split off and the
        remainder stays in pool reserves.
\end{itemize}

There is no separate LP-token-side fee on either operation. This keeps LP
accounting symmetric with the asset-to-asset path and avoids
double-charging when LP shares are the counterparty.
...
\texttt{swapMint}, \texttt{burnSwap}, \texttt{flashLoan}, and
\texttt{initialMint}. The action is one-way; there is no \texttt{unkill}.
Both \texttt{burn} (proportional LP redemption) and
...
\begin{table}[h]
\centering
\caption{Suggested $\kappa$ ranges by pool type (see
\texttt{security/\allowbreak drainage-guidelines.md})}
\label{tab:kappa}
\begin{tabular}{p{0.30\textwidth}lp{0.40\textwidth}}
\toprule
...
\begin{itemize}
  ...
  \item \texttt{checklist.md} --- the security review checklist (14
        ...
        in source) or \textsc{documented} (closed by docs); two
        operational rows --- multisig migration and paid external audit
        --- are recorded as \textsc{deferred with trigger} at named TVL
        thresholds, with compensating mitigations specified in
