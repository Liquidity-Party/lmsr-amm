...
(\texttt{LMSRKernel.\allowbreak swapAmountsForExactInput}) computes the
output $y$ via a two-pass Heun-style midpoint of the Hanson exact-input
formula. Writing $\tilde{b} := b_{\mathrm{mid}}$ and
$\tilde{r} := r_{\mathrm{mid}}$ for the pass-1 quantities:
...
\begin{align}
  ...
  &\qquad \tilde{b} = \kappa\,\Big(\Sm + \tfrac{a - y_0}{2}\Big),
   \quad \tilde{r} = \exp\!\big((q_j - q_i)/\tilde{b}\big), \nonumber\\
  &\qquad y = \tilde{b}\,\ln\!\Big(1 + \tilde{r}\,\big(1 - e^{-a/\tilde{b}}\big)\Big).
\end{align}
