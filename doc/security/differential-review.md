...
**Source authority:** `asset-authority-matrix.md` §B–§D (auth gates) + the per-function divergence analysis in this document.
...
The function list is the matrix in `asset-authority-matrix.md` §B–§D. We do not re-derive auth gates here; we _cite the matrix cell_ and discuss only the _divergence from the canonical design_. A divergence with no documented justification is by definition a finding; such findings are listed in §7.
...
- **`Funding.PREFUNDING`** — pool reads `balanceOf(pool, token) - cached - owed`. **Gate:** `require(msg.sender == payer)` at `:216`. This gate only blocks `payer`-spoof / allowance theft; the balance delta is unauthenticated and carries no depositor identity, so a `payer == msg.sender` caller can consume a stranger's prefund. PREFUNDING is therefore intended only for integrating contracts that bundle the deposit and call atomically in one tx (see `Funding.PREFUNDING`).
...
2. **Read-at-call-time recipient.** `protocolFeeAddress` is read inside the call, not snapshotted at accrual. Mirrors UniV3's `collectProtocol` semantic. Documented as intended on `setProtocolFeeAddress` (`PartyPool.sol:122-129`); closure of O-4 (resolved).
...
2. **`sweepEth` modifier.** Refunds residual `address(this).balance` at the end (`:128-135`). UniV2 router has the equivalent `refundETH()` that the caller must explicitly call; we do it implicitly. **Justification:** closes O-2 (Concierge ETH stickiness, resolved).
...
**No undocumented divergences.** Every entry above has a written justification. No new finding is generated by this review.
