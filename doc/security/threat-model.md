...
`tooling-runbook.md`, `differential-review.md`.
...
| **Integrator (read-only)**                | Read view functions (`balances`, `LMSR`, `IPartyInfo.price` / `poolPrice`, ERC20 getters).                                                                                                                                                              | Not gated. Read-only-reentrancy hazard documented in source banners on `IPartyPool` (`:24-37`) and `IPartyInfo` (`:8-17`); O-5 (resolved). Pool funds are not at risk; the hazard is integrator misuse. Checklist §C.2, §C.6, §H.4. |
...
| ABDK Q64.64            | `v3.2` (`lib/abdk-libraries-solidity`)                                                                                                                                                                                                                                                                                                                             | `int128` fixed-point math underpinning `LMSRKernel` (`fromUInt`, `mulu`, `divu`, `exp`, `ln`). Bounds-checked at every conversion site (checklist §E.3).                |
...
- **AMM economics.** JIT-LP and b-sandwich analyses in `economics-audit.md`
  ...
  18+ orders-of-magnitude tested; the payer-gate (invariant I-3) closes
  allowance/`payer`-spoof theft on the PREFUNDING path, but the PREFUNDING
  front-run race itself is accepted by design — PREFUNDING is intended only for
  integrating contracts that bundle the deposit and the call atomically in one
  tx (see §H.6 in `checklist.md` and `Funding.PREFUNDING`). Closure §H.1–H.11.
...
| I-3  | PREFUNDING payer-gate: `D_i` is consumable only by a `(payer, msg.sender)` pair with `msg.sender == payer`, so no `payer`-spoof / allowance theft is possible. This does NOT bind `D_i` to its depositor — a `payer == msg.sender` caller can still consume a stranger's non-atomic prefund; that front-run is accepted by design (PREFUNDING is atomic-same-tx-only). | Inline in `handler.prefundTheftAttempt()`.                                        |
...
- Commit a postmortem to `doc/security/`: timeline, decoded calldata, root
...
| N.3  | Written, tested incident-response plan                    | OPEN                                                                                                                                                                                                                                                                                                                     | Plan in this doc §9; no worked-example dry-run on record yet — run and record one before mainnet.                                                                    |
...
| N.8  | Key invariants defined and tested on every commit         | OK (manual gate — no CI by design): I-1..I-18 (17 functions, I-7 retired) + C-1..C-4 run under `forge test`. These are immutable deploy-once contracts with no post-release updates, so there is intentionally no `.github/` CI; the suite is run and must be green as a pre-deploy gate. |
| N.9  | Static analysis (Slither) in CI; no high-severity merges  | OK (manual gate — no CI by design, see N.8): `slither.config.json` exists; `slither .` is run and must be clean before each deploy. No GitHub-Actions workflow — deploy-once contracts gate at deploy time, not per-commit. |
| N.10 | External audit before mainnet                             | DONE: independent external audit completed by Jay Joshi (14 full days, all aspects of the system). See §11.N10.                                                                                                            |
| N.11 | Vulnerability disclosure / bug bounty channel             | KNOWN: `SECURITY.md` at repo root names primary channels (`tim@dexorder.com` + DM to @LiquidityParty on X), 48 h acknowledgement target, scope and out-of-scope clauses. No formal bounty cap today; promotion to Immunefi Boost scheduled at the same TVL trigger as N.7 / N.10. See §11.N11.                  |
...
and the completed independent external audit (N.10).
...
_Question:_ Has a paid external audit been completed before mainnet v1
deploy? Which auditor, what scope?

**Answer: DONE.** An independent external audit was completed by Jay Joshi,
who spent 14 full days reviewing all aspects of the system. This is
complemented by the in-house structured review documented under
`doc/security/`, the hardware-key + low-blast-radius admin model (N.6 / N.7),
and the TVL-capped launch posture.
...
**Recorded plan:** promote to Immunefi Boost at the same TVL trigger as N.7 / N.10.
...
- **DEFERRED (recorded above with trigger):** N.7 (multisig).
