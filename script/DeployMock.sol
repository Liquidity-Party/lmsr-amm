...
contract DeployMock is Script {
    ...
    function run() public {
        ...
        vm.startBroadcast(DEV_ACCOUNT_0);
...
//        wteth = new MockERC20('Wrapped Test Ether', 'WTETH', 18);
        wteth = new MockWrapper();

        // deploy a PartyPlanner factory and create the pool via factory
        NativeWrapper wrapper = wteth;
        IPermit2 permit2 = IPermit2(address(0));
        ...
        console2.log("payer (to newPool)  =", DEV_ACCOUNT_0);
        console2.log("wteth.balanceOf(payer)      =", wteth.balanceOf(DEV_ACCOUNT_0));
        ...
        planner.newPool(
            ...
            _feePpm,
            _feePpm,
            DEV_ACCOUNT_0, // payer: this script
        ...
        string memory conciergeStr = vm.toString(address(concierge));
        vm.setEnv('PLANNER', plannerStr);
        vm.setEnv('INFO', infoStr);
        vm.setEnv('CONCIERGE', conciergeStr);
        ...
        chainConfigStr = vm.serializeString('config', 'PartyConcierge', conciergeStr);
