...
contract DeploySepolia is Script {
    ...
    function run() public {
        ...
        console2.log('deploying concierge');
        ...
        vm.setEnv('CONCIERGE', vm.toString(address(concierge)));
    ...
    function exercise( IPartyPool pool, IPartyInfo info) internal {
        ...
        pool.swapMint(msg.sender, Funding.APPROVAL, msg.sender, 0, swapMintAmt, 0, 0, bytes(""));

        // 5) regular swap (token 0 -> last token)
        uint256 inputIndex = 0;
        uint256 outputIndex = n > 1 ? n - 1 : 0;
        uint256 maxIn = 89 * 10**6; // varied
        pool.swap(msg.sender, bytes4(0), msg.sender, inputIndex, outputIndex, maxIn, uint128(0), 0, false, '');
        ...
        pool.burnSwap(msg.sender, msg.sender, lpForBurnSwap, burnToIndex, 0, 0, false);
    }
}
